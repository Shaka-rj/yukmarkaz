...
def get_regions_for_route_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for viloyat in VILOYATLAR:
        # Callback format: "region:<viloyat_nomi>"
        builder.button(text=viloyat, callback_data=f"a:{viloyat}")
    builder.adjust(2)  # Qatorda 2 tadan tugma
    return builder.as_markup()

def get_regions_for_b_route_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for viloyat in VILOYATLAR:
        # Callback format: "region:<viloyat_nomi>"
        builder.button(text=viloyat, callback_data=f"b:{viloyat}")
    builder.adjust(2)  # Qatorda 2 tadan tugma
    return builder.as_markup()
...
def get_elon_pagination_keyboard_route(region_a: str, region_b: str, page: int, total_pages: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    
    # 1-qator: Navigation (Oldingi / Sahifa / Keyingi)
    nav_buttons = []
    if page > 1:
        nav_buttons.append(InlineKeyboardButton(text="⬅️ Oldingi", callback_data=f"page_r:{region_a}:{region_b}:{page - 1}"))
    else:
        nav_buttons.append(InlineKeyboardButton(text="⛔️", callback_data="noop"))
    ...
    if page < total_pages:
        nav_buttons.append(InlineKeyboardButton(text="Keyingi ➡️", callback_data=f"page_r:{region_a}:{region_b}:{page + 1}"))
    else:
        nav_buttons.append(InlineKeyboardButton(text="⛔️", callback_data="noop"))
    ...
    # 2-qator: Yangilash
    builder.row(InlineKeyboardButton(text="🔄 Yangilash", callback_data=f"refresh_r:{region_a}:{region_b}:{page}"))
    
    # 3-qator: Boshqa viloyatni tanlash
    builder.row(InlineKeyboardButton(text="🗺 Boshqa viloyatni tanlash", callback_data="change_region_r"))
